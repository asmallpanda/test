...
class NewWordDetect:
    ...
    def load_corpus(self, corpus_path):
        with open(corpus_path, encoding='utf-8') as f:
            for line in f:
                sentence = line.strip()
                for word_len in range(1, self.max_word_length):
                    self.ngram_count(sentence, word_len)
        return

    # 按照窗口长度取词，并记录左邻右邻
    def ngram_count(self, sentence, word_len):
        for i in range(len(sentence) - word_len + 1):
            word = sentence[i:i + word_len]
            self.word_count[word] += 1
            if i - 1 >= 0:
                char = sentence[i - 1]
                self.left_neighbor[word][char] = self.left_neighbor[word].get(char, 0) + 1
            if i + word_len < len(sentence):
                char = sentence[i + word_len]
